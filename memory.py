...
class ReplayMemory():
    ...
    def _get_sample_from_segment(self, segment, i):
        ...
        R = torch.tensor([sum(self.discount ** n * transition[self.history + n -
                                                              1].reward for n in range(self.n))], dtype=torch.float32, device=self.device)
    ...
    def sample(self, batch_size):
        ...
        actions, returns, nonterminals = torch.cat(
            actions), torch.cat(returns), torch.stack(nonterminals)
